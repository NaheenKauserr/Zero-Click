...
def chart_region(df, ct):
    ...
    ax.set_title("Sales by Region", fontsize=10, fontweight="bold",
...
def chart_category(df, ct):
    ...
    ax.set_title("Sales by Category", fontsize=10, fontweight="bold",
...
def chart_trend(df, ct):
    ...
    ax.set_title("Monthly Sales Trend", fontsize=10, fontweight="bold",
                 color="#15803D", pad=8, loc="left")
    ax.set_ylabel("Sales", fontsize=7, color="#6B7280")
...
def chart_scatter(df, ct):
    ...
    ax.set_title("Sales vs Profit", fontsize=10, fontweight="bold",
                 color="#C2410C", pad=8, loc="left")
    ax.set_xlabel("Sales", fontsize=7, color="#6B7280")
    ax.set_ylabel("Profit", fontsize=7, color="#6B7280")
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: _fmt(x)))
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: _fmt(x)))
    _style_ax(ax)
    fig.patch.set_facecolor("white")
...
def chart_orders(df, ct):
    ...
    ax.set_title("Monthly Orders Trend", fontsize=10, fontweight="bold",
                 color="#1D4ED8", pad=8, loc="left")
    ax.set_ylabel("Orders", fontsize=7, color="#6B7280")
...
def chart_heatmap(df, ct):
    ...
    ax.set_title("Seasonality Heatmap", fontsize=10, fontweight="bold",
                 color="#7C3AED", pad=8, loc="left")
...
def generate_report_pdf(df, kpis, ml_results, forecast_data, insights, charts) -> bytes:
    ...
    story.extend(_section_hdr("Sales Analytics — Charts", ST))

    chart_fns = [
        ("Sales by Region",      lambda: chart_region(df, ct)),
        ("Sales by Category",    lambda: chart_category(df, ct)),
        ("Monthly Sales Trend",  lambda: chart_trend(df, ct)),
        ("Sales vs Profit",      lambda: chart_scatter(df, ct)),
        ("Monthly Orders Trend", lambda: chart_orders(df, ct)),
        ("Seasonality Heatmap",  lambda: chart_heatmap(df, ct)),
    ]
    chart_imgs = []
    ...
    # ── Page 3: Top Products ──────────────────────────────────────────────────
    story.append(PageBreak())
    story.extend(_section_hdr("Top Products by Sales & Profit", ST))
...
def generate_report_excel(df, kpis, ml_results, forecast_data, insights) -> bytes:
    ...
    _xl_title(ws3, "Sales by Region", end_col=4, accent=acc3)
    rbr = sales_by_region(df, ct)
    _write_df_xl(ws3, rbr, start=2, accent=acc3)
    if len(rbr) >= 2:
        ch = BarChart(); ch.title = "Sales by Region"; ch.style = 10
        ch.width = 16; ch.height = 9
    ...
    _xl_title(ws4, "Sales by Category", end_col=5, accent=acc4)
    sbc = sales_by_category(df, ct)
    ...
    _xl_title(ws5, "Top 10 Products by Sales & Profit", end_col=6, accent=acc5)
    ...
    _xl_title(ws6, "Monthly Sales Trend", end_col=5, accent=acc6)
    mt = monthly_trend(df, ct)
    if mt.empty:
        ws6.cell(row=3, column=1, value="No datetime column found.")
    else:
        _write_df_xl(ws6, mt, start=2, accent=acc6)
        ch = LineChart(); ch.title = "Monthly Sales Trend"; ch.style = 10
        ch.width = 16; ch.height = 9
